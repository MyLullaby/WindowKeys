...
上、左、右三个方向动作优先触发目标应用的 macOS 原生窗口菜单命令，保留系统动画、
平铺边距和台前调度行为。命令不可用或未生效时直接调整窗口，不主动展开应用菜单。
下键只写窗口位置，绝不写窗口大小；C 负责调整大小并居中。调整后会核对实际位置和尺寸，
处理系统平铺恢复覆盖窗口写入的情况；新动作会取消旧动作的动画和后续校正。
C 直接应用目标大小和位置，不播放逐帧缩放动画；修正时尽量在同一轮完成位置和大小调整。
菜单栏中的“自定义窗口动画”只控制下键的居中平移动画；原生窗口命令的系统动画不受影响。
...
脚本调用当前源码的窗口控制器，检查最大化、退出平铺后的缩放、单次居中和动画中断，
